...
class Tracker(object):
    def __init__(self, r):
        ...
        self.morph_elem = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,
                                 (kern_size, kern_size),
    ...
    def make_seed(self, image, padding=25):
        ...
        med = cv2.medianBlur(blur_frame, 7)
        # cv2.imshow("med", med)

        morph = cv2.dilate(med, self.morph_elem)
        morph = cv2.erode(morph, self.morph_elem)

        seed = cv2.adaptiveThreshold(morph, 255,  cv.CV_ADAPTIVE_THRESH_MEAN_C, cv.CV_THRESH_BINARY_INV, 151, 15)
